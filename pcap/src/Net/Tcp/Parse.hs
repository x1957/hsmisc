...
pTcpOptionalHeader offset = replicateM offset anyByte >>= return . TcpOptionalHeader :: Parser TcpOptionalHeader

pTcpPacket = do { th <- pTcpHeader
                ; oh <- let len = offset th
                        in if len > 5
                           then (pTcpOptionalHeader $ (fromEnum len - 5) * 4) >>= return . Just
                           else return Nothing
                ; td <- many anyByte
